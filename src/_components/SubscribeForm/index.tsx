...
const SubscribeForm = ({
  ...
  const handleSubmit = (e: any) => {
    ...
    // ✅ Only send phone if actually provided
    if (sanitizedPhone) {
      // If your API needs + prefix, keep this:
  ...
  const handleChange2 = (value: string) => {
    ...
    if (numericValue.length <= 3) {
      ...
      formattedPhoneNumber = `(${numericValue.slice(0, 3)}) ${numericValue.slice(
        3,
        6
      )}-${numericValue.slice(6, 11)}`
    }
